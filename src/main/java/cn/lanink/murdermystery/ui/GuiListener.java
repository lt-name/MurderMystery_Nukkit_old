...
public class GuiListener implements Listener {
    ...
    public void onPlayerFormResponded(PlayerFormRespondedEvent event) {
        ...
        String uName = MurderMystery.getInstance().getConfig().getString("插件命令", "killer");
        String aName = MurderMystery.getInstance().getConfig().getString("管理命令", "kadmin");
        if (event.getWindow() instanceof FormWindowSimple) {
            ...
            if (event.getFormID() == GuiCreate.USER_MENU) {
                switch (simple.getResponse().getClickedButtonId()) {
                    case 0:
                        MurderMystery.getInstance().getServer().dispatchCommand(player, uName + " join");
                        break;
                    case 1:
                        MurderMystery.getInstance().getServer().dispatchCommand(player, uName + " quit");
                        break;
                    case 2:
                ...
                switch (simple.getResponse().getClickedButtonId()) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        MurderMystery.getInstance().getServer().dispatchCommand(player, aName + " reload");
                        break;
                    case 5:
                        MurderMystery.getInstance().getServer().dispatchCommand(player, aName + " unload");
                        break;
                }
            }
